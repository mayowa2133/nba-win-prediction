#!/usr/bin/env python
"""
star_ladder_preview.py

For a given player, estimate the probability they hit various
points thresholds (15+, 20+, 25+, 30+, etc.) in their NEXT game.
...
Example:
  ...
  python star_ladder_preview.py \
    --player "Devin Booker" \
    --thresholds "15,20,25,30,35" \
    --season-min 2023

Optional manual matchup override (like predict_points_over_line_next_game.py):

  python star_ladder_preview.py \
    --player "Luka Doncic" \
    --thresholds "20,25,30,35,40" \
    --opp DAL --home-away home
"""
...
def print_ladder_table(rows: List[Dict[str, Any]]):
    ...
    """
    print("\n=== Points ladder (P(points >= threshold)) ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="Preview a star's points ladder probabilities for their NEXT game."
    )
    parser.add_argument(
        "--player",
        required=True,
        help="Player name (e.g. 'Luka Doncic')",
    )
    parser.add_argument(
        "--season-min",
        type=int,
